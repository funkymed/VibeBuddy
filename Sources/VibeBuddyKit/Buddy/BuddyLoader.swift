...
public enum BuiltInBuddy {
    ...
    #          recule) · bored (une fois sur deux vers le haut) · scan
    #          (gauche/droite, plus quelques haut/bas) · wander (les quatre
    #          directions) · dart (wander, deux fois plus vite)
    #
    ...
    # ce sont les scanlines allumées qui dessinent sa surface. Il fait 62×30, soit
    # 31×15 cellules à 2 pt par pixel — la pastille repliée est mesurée dessus, donc
    # chaque point de large ici en coûte deux à l'encoche.
    #
    # Il faisait 80 de large : mesuré, 16 à 20 pt ne s'allumaient jamais de chaque
    # côté. Le resserrement s'accompagne d'un « gap » réduit, sinon le regard perd
    # son débattement — EyeRaster borne le déplacement à
    # « largeur/2 − écart − demi-œil », donc rendre l'écart aux yeux leur rend le
    # déplacement. À 62 avec gap:10 ils vont toujours à 12 pt, comme à 80 avec
    # gap:16. C'est le maximum :
    ...
    eye   shape:arc w:22 h:10 t:0.34 bend:1 gap:5 y:0
    ...
    eye   shape:oval w:13 h:9 r:4.5 tilt:12 gap:10 y:0
    # mouth shape:oval w:14 h:5 r:2.5 y:8
    time  beat:1.2 blink:0.2 grain:0.012 glitch:0 gaze:scan

    # En attente : petits yeux qui regardent souvent en l'air — il attend, et ça
    ...
    # Terminé : la forme et le rythme d'« awaiting », mais l'œil bien plus grand —
    # c'est ce qui les sépare, puisqu'ils partagent la couleur. Deux arches épaisses
    ...
    failed (red #FF5555)
    eye   shape:x w:14 h:14 t:0.30 gap:9 y:0
