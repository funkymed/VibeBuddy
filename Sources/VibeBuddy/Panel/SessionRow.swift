...
struct SessionRow: View, Equatable {
    ...
    /// Told which session was clicked, so the face can take that session's state.
    ...
    /// Under the pointer and clickable.
    private var isHot: Bool { hovering && jumpPID != nil }

    private var session: AgentSession { group.primary }

    // nonisolated: a View is MainActor-isolated, and an Equatable conformance that
    // crosses that boundary is a data race under Swift 6.
    ...
    var body: some View {
        ...
        if let pid = jumpPID {
            Button {
                ...
                .buttonStyle(.plain)
                // A solid shape for the hover test, not the union of whatever is opaque
                // inside: without it the highlight blinks as the pointer crosses the
                // gaps between the three lines of text.
    ...
    /// Column widths for the right-hand side, so two rows line up. They floated: the
    /// gauge, the age and the mode badge were laid out end-to-end after a `Spacer`, each
    /// as wide as its own text, so a session with no permission mode pulled its gauge 70
    /// pt to the right of its neighbour's.
    private enum Column {
        /// Ring plus percentage.
    ...
    private var content: some View {
        // The three lines of text are top aligned; everything on either side of them —
        // the state dot, the gauge, the age, the mode — is centred against the row's
        // full height.
        HStack(alignment: .top, spacing: 12) {
            // Centred like the columns on the other side: the dot is the session's
            // state, a fact about the whole row rather than about the line it happens
            // to sit next to.
            statusDot
                .frame(maxHeight: .infinity, alignment: .center)

            // Eight, not four. The reference card is 113 px tall — about 125 pt — for
            // the same three lines we were fitting into 70.
            VStack(alignment: .leading, spacing: VibeTheme.Spacing.s) {
                ...
                HStack(spacing: 8) {
                    // One secondary grey, not two. Sampled off the reference, « opus 5
                    // », « depuis 6h46 » and the path all come out at `(90,95,102)` — a
                    // single level, around 37 % white. Ours put the first two at 60 %
                    // and the path at 35 %, so the model and the duration competed with
                    // the project's name.
            ...
            // Centred against the whole row, while the left side stays top aligned.
            // These three are facts about the session as a whole, not about any one of
            // its three lines — pinned to the top they read as belonging to the
            // project's name.
            HStack(spacing: 12) {
                ...
                // The slot is reserved even when the mode is missing — that is the
                // whole point of a column.
                Group {
        ...
        // The same block every other surface of the panel is drawn as: one radius, one
        // fill, one hairline.
        ...
        // The same pooled edge the buttons wear, so a lit row and a lit answer are
