...
/// The jump is an equality on the tty, so these tests check the two halves of
/// that equality: that the kernel side produces a `/dev/ttysNNN`, and that the
/// script side asks for exactly that string.
@Suite("Terminal jump")
struct TerminalJumperTests {

    @Test("launchd has no controlling terminal")
    func daemonHasNoTTY() {
        // pid 1 is the one process guaranteed to exist and guaranteed to have no
        // tty, which makes it the only portable negative case.
        #expect(ProcessLookup.tty(of: 1) == nil)
    }

    @Test("a tty, when there is one, is a device path")
    func ttyLooksLikeADevice() throws {
        // The test runner may or may not own a terminal — under `swift test` in
        // a shell it does, in CI it does not. Both are correct; only the shape
        // is asserted, and only when there is something to assert.
    ...
    // The tty is interpolated into the script, and a mismatch here is a jump
    // that silently lands on the wrong tab — the one failure this design exists
    // to rule out.
    ...
    func itermScriptCarriesTTY() {
        ...
        // Window, tab and session all get selected: selecting the session alone
        // changes tab without ordering the window forward.
    ...
    // Ghostty, kitty and Alacritty ship no scripting dictionary. Claiming them
    // would produce a jump that fails at click time instead of one that falls
    // back to activating the application.
